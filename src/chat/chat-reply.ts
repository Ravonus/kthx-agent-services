...
export const buildIntentAndReplyDrilldownPrompt = ({
  ...
  const lines = [
    ...
    "Do not invent route/action names. Use only names present in the capability map.",
    "Do not claim you ran a route unless the result is present in the provided runtime context.",
    "If user names a non-existent route, acknowledge it briefly and run the closest real route instead.",
    "",
    "Never disclose internal runtime details, logs, tokens, bridge status, permissions internals, queue state, directives, or infrastructure.",
