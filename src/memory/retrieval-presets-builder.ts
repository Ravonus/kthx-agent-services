import { buildTopParticipantsPreset } from "./retrieval-presets-top-participants.js";
import { collectLatestEvents } from "./retrieval-presets-events.js";
import type {
  RetrievalPresetBuilderInput,
  RetrievalPresetDoc,
...
type RetrievalPresetIntent = RetrievalPresetBuilderInput["intent"];
...
const DEFAULT_RANGE_MS_BY_INTENT: Record<RetrievalPresetIntent, number> = {
...
export const buildRetrievalPresets = (
  input: RetrievalPresetBuilderInput,
): RetrievalPresetSummary => {
  ...
  const defaultRangeMs =
    typeof input.defaultRangeMs === "number" && Number.isFinite(input.defaultRangeMs)
      ...
      : (DEFAULT_RANGE_MS_BY_INTENT[input.intent] ?? 24 * 60 * 60 * 1000);
  ...
  let mostRecentPost: RetrievalPresetSummary["mostRecentPost"] = null;
  ...
  if (requested.topParticipants) {
    topParticipants = buildTopParticipantsPreset({
      docs,
      nowMs,
      range,
      scopePostId,
